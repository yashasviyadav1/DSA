...
class Solution {
    public int minCost(int n, int[] cutsArr) {
        ...
        for(int i=c; i >= 1; i--){
            for(int j=i; j <= c; j++){ // i should never be on right of j thats why we can run j from i ownwards and not from 0 (this way we can get rid of the i > j condition from loop)
                int minCost = (int)1e9;
                for(int k=i; k <= j; k++){
