    ...
    while(!set.empty()){
        ...
        for(auto neighPair:AdjList[frontNode]){
            ...
            if(minDist[frontNode] + weight < minDist[neigh]){
                ...
                //[m-1] now after a node's distance to reach is updated, we need to update that 'neigh''s min distance in the set also, so for that (find if there already exist a node 'neigh' in the set, if yes then delete it) and then insert the new pair into the set
                // m-1 : this below lines will take O(logN) to find and del it 
                ...
                /*//[M-2] we can also do dome thing like shown below (but this will take O(n) time 
