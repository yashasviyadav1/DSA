...
class Solution {
    public boolean isSameTree(TreeNode p, TreeNode q) {
        if(p == null && q == null) return true; // both null -> same tree
        else if(p == null || q == null) return false; // only 1 is null -> not same
         // is curr nodes are same val, then their left and rigth subtrees need to be same as well for both trees to be same
        else if(p.val == q.val)
