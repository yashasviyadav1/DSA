// (Solved)✔️- Find duplicate subtrees - 1 approach 

// QOTD - 28 Feb 23 
/*✔️⭐ Approach - 1 (using string representation concept + unordered_map)
      ...
      ☑️solved at leetcode : https://leetcode.com/problems/find-duplicate-subtrees/
*/  
...
private:
    ...
    string stringRepWithFreq(TreeNode* root, vector<TreeNode*> &dupliSubtrRoots, unordered_map<string, int> &stringToFreq){
        ...
        strRepresentation += '(';
        strRepresentation += stringRepWithFreq(root -> left, dupliSubtrRoots, stringToFreq);
        strRepresentation += ')';
        strRepresentation += to_string(root -> val); // note : do not do ( strRep += int ) this will store the asci of int into string.
        strRepresentation += '(';
        strRepresentation += stringRepWithFreq(root -> right, dupliSubtrRoots, stringToFreq);
        strRepresentation += ')';
