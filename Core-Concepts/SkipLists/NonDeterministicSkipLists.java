...
/**
 * A node in the Skip List that stores data and references to next nodes at various levels.
...
class Node<T>{
     ...
     * @param level the maximum level (height) of the node, determines the size of the next array
     */
    @SuppressWarnings("unchecked")
    public Node(T data, int level){
        this.data = data;
        next = (Node<T>[])new Node[level + 1]; // if for a node level is 11 i.e from 0 to 11 then we need to have 12 size
    }
}

/**
 * A probabilistic/Non deterministic skip list implementation that provides average O(log n) time complexity
 * for search, insertion, and deletion operations.
 * 
 * <p>The skip list maintains multiple levels of linked lists, with higher levels containing
 * fewer elements and acting as "express lanes" for faster searching.</p>
 * 
 * @param <T> the type of elements maintained by this skip list, must be Comparable
 */
class SkipList<T extends Comparable<T>>{
    ...
    public void insert(T key){
        ...
        // we have alrady stored the predecessors in array, are are on level 0, so move curr to the next element to make sure it is not a duplicate key case 
        curr = curr.next[0];
        if(curr == null || !curr.data.equals(key)){
            // if this new height h is more then the longest node of the alrady existing list with height (h-y), then for the remaining y levels, we need to assign head node as predecessor for the new node 
            int levelForNewNode = getRandomHeight();
            if(levelForNewNode > currentLevel){ // this new nodes height is more then current maxumum height of a node, add head as predecessors for this node
    ...
    public void delete(T key){
        ...
        if(curr != null && curr.data.equals(key)){
            ...
            // going 0 to h level 1 by 1 and for each level of predecessor when predecessor point to curr, change it
            // the moment we find a predesesor that doesnpt point to curr, then no need to go above, as we consumed comlete height
        ...
        // if after deletion of nodes at multiple level if somehow the max height has been now reduced, just decrease the list's level
...
public class NonDeterministicSkipLists {
    ...
    public static void main(String[] args){
        ...
        /*
            - since this is a undeterministic implementation of skip lists i.e structure is different of the skip list even if we insert nodes in same order
