...
/* ✔️⭐Approach - 1 (using 2 while loops)
...
class Solution{
    ...
    private:// Creating a function to convert character into int (not asci value) implicitely 
    ...
    public: // main function
        string addBinary(string a, string b) {
            ...
            // when both strings are not consumed
            while(i >= 0 && j >= 0){
                ...
                int shortSum = charToInt(a[i]) + charToInt(b[j]) + power;

                cout << "shortSum : " << shortSum << endl;
                if(shortSum == 2){
                    sum += '0';
                    power = 1;
                }
                    ...
                    sum += '1';
                    power = 1;  // coz if short sum is 3 then binary of 3 is '11' and 1 is saved to sum, and 1 goes to power
                }
                else{
            ...
            // when b string is consumed and a is not consumed
            while(i >= 0){
                int shortSum = charToInt(a[i]) + power;
            ...
            // when a string is consumed and b is not consumed
            while(j >= 0){
                int shortSum = charToInt(b[j]) + power;
