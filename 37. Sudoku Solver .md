...
### Solved at : [Leetcode](https://leetcode.com/problems/sudoku-solver/description/)
...
class Solution {
    ...
    /*✔️⭐Approach - 1 (backtracking, recursion)
...
private:
    // Fun.3 - isSafeToPlace() returns true if we can place the value at row, col else returns false
    bool isSafeToPlace(vector<vector<char>> &board, int row, int col, int value){
        ...
        for(int i =0; i < n; i++){
            ...
            // step 2.1 : complete row check, if we find charValue in any cell of the row, return false
            if(board[row][i] == charValue) return false;
            // step 2.2 : complete coloumn check, if we find charValue in any cell of the col, return false
            if(board[i][col] == charValue) return false;
            //step 2.3 complete grid check, if we find charValue in any cell of the grid, return false
            ...
            for(int col = 0; col < n; col++){
                ...
                if(board[row][col] == '.'){
                    ...
                    for(int val = 1; val <= 9; val++){
                        ...
                        // step 4 : call fun.3 : isSafe(), if at the current row and col coordinated, its safe to place val, then
                        if(isSafeToPlace(board, row, col, val)){
                    ...
                    // step 9 : agar sari values dalke dekhliya aur koi value safe nahi mili, to matlab return kardo false, ke koi solution nahi mila
                    return false;
                }
            }
        }
